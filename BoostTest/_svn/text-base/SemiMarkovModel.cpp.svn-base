...
void SemiMarkovModel::SetupMatrices()
{
    double dtHalf = dt / 2;
		...
		//State Probabilities
	phi = vector<matrix<double>>(N+1);
	C = vector<matrix<double>>(N+1);
	W = vector<matrix<double>>(N+1);
	helper = vector<matrix<double>>(N+1);
	w = vector<vector<double>>(N+1);
    ...
    for (int i = 1; i < N + 1; i++)
	{
       ...
       // NoWaitingTedious(i, W, w);		//TESTMODEL("W[i]: ", W[i])
		NoWaitingPolyWbl(i*dt, 	W[i]);
...
void SemiMarkovModel::ComputeStateProbabilities()
{
    double dtHalf = dt / 2;
	...
	boost::shared_ptr<boost::thread> m_thread =
		boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&SemiMarkovModel::DisplayTimeStep, this)));


	for (timeStep = 1; timeStep < N + 1; timeStep++)
    {
